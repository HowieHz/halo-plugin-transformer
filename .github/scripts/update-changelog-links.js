...
const REPO_NAME = "halo-plugin-transformer";
const REPO_PATH_RE = /^https:\/\/github\.com\/howiehz\/halo-plugin-transformer\//iu;

const DEFAULT_CHANGELOG_FILES = [path.join(process.cwd(), "CHANGELOG.md")];

const CHANGELOG_HEADING_RE = /^## \[(\d+\.\d+\.\d+)\]\s*-\s*/gmu;
const TRAILING_LINK_LINE_RE =
  /^\[(?:Unreleased|unreleased|\d+\.\d+\.\d+)\]:\s*https:\/\/github\.com\/howiehz\/halo-plugin-transformer\/(?:compare\/\S+|releases\/tag\/\S+)\s*$/iu;
...
function buildCompareLinks(versions) {
  ...
  links.push(
    `[${oldestVersion}]: https://github.com/${REPO_OWNER}/${REPO_NAME}/releases/tag/v${oldestVersion}`,
  );
  return links;
}

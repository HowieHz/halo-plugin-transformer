# 更新日志

<!--
三级标题顺序

### 💥 破坏性变更
### 🚀 新功能
### 🔧 代码重构
### ⚠️ 弃用
### 🗑️ 移除
### 🐛 错误修复
### 🔒 安全
### 📄 文档
### 🛠️ 杂项维护
-->

本项目的所有显著变更将记录在本文件中。

该格式基于[保持变更日志](https://keepachangelog.com/en/1.1.0)，
本项目遵循[语义版本管理](https://semver.org/spec/v2.0.0.html)。

## [Unreleased]

## [1.0.7] - 2026-08-17

### 💥 破坏性变更

- Halo CMS 最低支持版本调整为 2.26.0。

### 🛠️ 杂项维护

- 管理端构建从 Rsbuild 迁移至 Vite，并使用 ESM 输出。
- 更新项目依赖。

## [1.0.6] - 2026-08-06

### 🛠️ 杂项维护

- 更新依赖。

## [1.0.5] - 2026-06-17

### 🛠️ 杂项维护

- 更新依赖。

## [1.0.4] - 2026-06-15

### 💥 破坏性变更

- Halo CMS 最低支持版本调整为 2.25.0。

### 🛠️ 杂项维护

- 更新依赖。

## [1.0.3] - 2026-05-25

### 🛠️ 杂项维护

- 更新依赖。

## [1.0.2] - 2026-04-22

### 💥 破坏性变更

- Halo CMS 最低支持版本调整为 2.24.0。

### 🚀 新功能

- 新增转换规则兼容性排查功能，可在批量选择规则后逐步缩小问题范围，并在排查结束后恢复排查前的规则启用状态。

### 📄 文档

- 优化 UI 文案。
- 将插件扩展定义的显示名称和描述调整为中文。

### 🛠️ 杂项维护

- 更新依赖。

## [1.0.1] - 2026-04-13

### 🔧 代码重构

- 优化代码结构。

### 📄 文档

- 更新贡献指南和发布说明文档，补充 PR 检查与正式版发布流程。

## [1.0.0] - 2025-04-10

### 🚀 新功能

- 第一个版本发布。

[Unreleased]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.7...HEAD
[1.0.7]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.6...v1.0.7
[1.0.6]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.5...v1.0.6
[1.0.5]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.4...v1.0.5
[1.0.4]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.3...v1.0.4
[1.0.3]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.2...v1.0.3
[1.0.2]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.1...v1.0.2
[1.0.1]: https://github.com/HowieHz/halo-plugin-transformer/compare/v1.0.0...v1.0.1
[1.0.0]: https://github.com/HowieHz/halo-plugin-transformer/releases/tag/v1.0.0

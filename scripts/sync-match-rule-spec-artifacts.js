...
export const MATCH_RULE_NODE_SPECS = ${formatTsLiteral(data.nodeTypes)} as const;

export const MATCH_RULE_ENUM_SPECS = ${formatTsLiteral(data.enumValues)} as const;
...
function formatTsLiteral(value, indentLevel = 0) {
  if (Array.isArray(value)) {
    return `[${value.map((item) => formatTsLiteral(item, indentLevel)).join(", ")}]`;
  }
  ...
  if (value && typeof value === "object") {
    ...
    const body = entries
      .map(([key, entryValue]) => `${childIndent}${formatTsKey(key)}: ${formatTsLiteral(entryValue, indentLevel + 1)},`)
      .join("\n");
    return `{\n${body}\n${indent}}`;
  }

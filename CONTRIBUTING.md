...
- `.github/workflows/ci.yaml`
  ...
  - 该 workflow 会先执行 `release-policy`，再执行插件构建检查；最后由聚合 job `ci` 给出受保护分支使用的最终状态
  - 普通 PR 可以照常改代码，但不能修改 `gradle.properties` 里的 `version`
  - 只有带 `release` label 的 PR 才允许修改 `gradle.properties` 里的 `version`，且版本号必须是递增的语义化版本

...
`README.md` 面向插件使用者，重点讲“能做什么、怎么配置、有什么运行时表现”；这份文档面向贡献者，重点讲“怎么构建、哪些是规范源、架构边界在哪里、实现细节为什么这样设计”。
...
- `README.md`
    - 面向用户
    - 说明功能、配置方式、规则能力、导入导出和运行时表现
- `CONTRIBUTING.md`
    - 面向贡献者
    - 说明构建链路、spec 生成链路、架构约定、平台边界与实现细节
...
- 规则若当前停留在高级模式：
    ...
    - 仍然无效的 JSON 草稿会继续按 `JSON_DRAFT` 导出，避免把错误悄悄改写成另一棵规则树
...
为了减少“填了半天，结果保存才发现不合法”的情况，校验明确分成三层：编辑时、导入时、保存时。  
这么做的原因不是重复劳动，而是分别守住三类边界：
...
- 同时，控制台列表读模型、单项读写接口、排序清洗也必须过滤 deleting 资源；这不是 UI 小修补，而是 `finalizer` 生命周期在读写两侧的一致性约束

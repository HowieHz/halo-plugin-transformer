...
这份文档主要写给维护者和贡献者看。  
如果您主要关注插件能力、配置方法和运行效果，请先阅读 `README.md`。  
如果您准备修改代码、补测试、调整设计，或需要理解当前实现的约束与取舍，再阅读本文件会更合适。
...
- 改控制台状态、草稿、路由同步：看[“前端状态模型”](#前端状态模型)和[“控制台交互约束”](#控制台交互约束)
- 改规则树、Schema、生成文件：看[“匹配规则规范”](#匹配规则规范)
- 改导入导出结构或导入后的处理方式：看[“导入 / 导出协议”](#导入--导出协议)和[“校验分层”](#校验分层)
- 改运行时装载、过滤、删除链路：看[“运行时架构约定”](#运行时架构约定)
- 改左侧排序、保存冲突：看[“排序模型”](#排序模型)和[“并发控制”](#并发控制)
...
如果新增文件，优先放进已有边界里；生成代码与手写状态逻辑应分层放置，跨组件共享逻辑也应放入职责明确的模块中。
...
如果后续新增代码，优先落入现有职责边界清晰的包中，避免引入职责范围宽泛的聚合包。

- `extension`
  - Halo 扩展资源模型，只承载持久化字段与资源自身的最小领域语义
- `endpoint`
  - 控制台读写接口、读取模型和排序接口编排
- `rule`
  - `match-rule` 领域模型、布尔化简和规则语义本身
- `validation`
  - 保存期领域校验与对应异常
- `runtime`
  - 运行时读取模型、规则解析和执行前收敛逻辑
- `runtime.store`
  - 由 Halo `watch` 驱动的内存快照
- `runtime.transform`
  - HTML / 模板注入的具体变换原语，不承担 WebFlux 入口职责
- `template`
  - Thymeleaf 处理器适配层
- `service`
  - 当前只放代码片段删除生命周期相关协调逻辑
- `support`
  - 乐观并发、上下文桥接、引用 id 规范化等基础辅助
- `contract.generated`
  - 从 `specs/` 生成的契约辅助代码；它是生成物，不参与手写领域建模

## 前端状态模型

前端实现需要优先保持状态边界清晰。  
“列表数据、编辑草稿、提交参数”若被混合维护，会放大后续演进和调试成本。
...
- 读取模型（`ReadModel`）
  ...
  - 常规刷新必须整体替换这份快照；不再分别刷新列表和 `snippet-order / rule-order`
- 编辑草稿（`EditorDraft`）
...
- 前端可为了编辑体验提前整理 `snippetIds`（例如 `trim`、去空、去重）
  - 但这只是 UX 层便利，不是权威规范化
  - `snippetIds` 的 authoritative normalization 必须只在后端写链路执行，并被删除协调器与运行时读取复用；同一规则的规范化语义不再分散到前端或多个后端模块分别维护

这样做的目的很明确：  
写接口不应混入 `id`、临时编辑状态、JSON 草稿等前端态字段。
...
- 前端应优先通过共享 snapshot controller 统一承载这份快照；页面层不再并排维护 `itemsResp`、`orders`、`orderVersion`
...
- 控制台左侧列表如果同时依赖“资源本体 + 排序资源”，前端读取时必须把它们收敛成同一次快照（snapshot）
  ...
  - 排序资源的唯一写真源仍然只保留在 `snippet-order` / `rule-order`；不再为了刷新一致性把顺序冗余写回资源本体
...
- 新建代码片段 / 转换规则弹窗的“导入来源编排”（source modal、剪贴板读取、文件选择、通用失败提示）必须复用共享 composable；两个弹窗不再分别维护独立 source flow
- 跨环境关系迁移若后续支持，应单独设计显式协议，而不是把关系字段直接并入当前导入导出内容（transfer）
- 新建 / 编辑 / 导入（create / edit / import）这三条写路径必须复用同一份前端校验器（validator）；弹窗、右侧编辑器和导入提示不再复制校验条件
- 新建弹窗同样要复用共享草稿控制器（draft controller），把默认草稿（`baseline`）、是否已修改（`dirty`）和提交快照（`submit snapshot`）统一起来；这些逻辑不再回收到组件内部重新实现
- 转换规则的 create / edit 表单语义（例如 `REMOVE` 时显示哪些字段、空关联 warning、snippet 勾选派生）应复用共享 composable；弹窗和右侧编辑器不再分别叠加条件胶水
...
- 中间编辑器与右侧关系面板在当前标签页（tab）下都必须从同一份活动编辑草稿（`EditorDraft`）派生；右侧关系面板不能回退到已保存列表，避免左右两侧语义不一致
- Transformer 页面语义本身只允许由一份共享 page session controller 表达 `single / create / bulk`
  - `activeTab`、当前页面模式和 remembered selection 的迁移必须统一收口
  - 不再在 `TransformerView` 里手工拼 `createModalTab + bulkMode + selectedId` 去反推当前页面语义
- `bulk / create` 只会隐藏当前显示的选中项，不会清掉之前记住的选中项；退出这些页面语义后，应能回到该标签页（tab）上次打开的资源
- 新建弹窗用单一当前活动标签（`ActiveTab | null`）表达状态，不再维护两颗互斥布尔；避免出现代码片段 / 规则弹窗同时为真，或者两边都没收好口的隐式组合态
- 多步流程弹窗（例如批量导入）只用一个流程状态对象表达 `source/options/result`，不再并列维护多颗“是否显示 / 结果 / 进行中”状态去手动拼阶段
- 地址栏里的页面语义只改当前标签页（tab）；`mode=bulk`、`action=create` 这类 URL 状态不应影响另一个标签页已记住的选中项

## 控制台交互约束

下面这些交互规则已经定下来了；如果后续需要调整，应整体评估整条语义，而不是只改其中一个按钮或某个局部状态：
...
- 常规列表刷新必须走聚合快照读口，把列表、排序映射和排序版本一起替换；“刷新列表”和“刷新排序”不再拆成两个独立入口
...
- “启用 / 禁用”走独立启停接口，只修改已保存资源的 `enabled`；不会同时保存右侧未保存草稿，也不会覆盖当前草稿页
...
- 无障碍约束：
  ...
  - 只在鼠标悬停时出现的提示，键盘聚焦时也必须可见；操作指引不应仅依赖 hover 显示
...
如果您修改了 `match-rule` 规范，不应只更新单一位置。  
...
- `GROUP` 缺少 `children` 时会补成空数组 `[]`，并继续提示“不能有空组”，不会隐式补入默认子规则
...
能复用 Halo 原生能力的场景，应优先复用平台机制，而不是在插件内额外构建同类能力。

这样做的目的在于提高稳定性、可维护性，并减少第二套状态或补丁式逻辑的增长空间。
...
- 插件主生命周期只应管理自己显式聚合的控制器集合（controllers）；不直接注入容器里的全量 `Controller` 列表统一 start/stop
- 运行时缓存优先做成“运行时内存快照”，由 Halo `watch`（监听）驱动维护，而不是在请求进来时按 TTL 回源重查
- 控制台若复用 watch-driven 快照作为读模型，写口成功后也必须同步把已保存资源回灌进这份可见快照；不将当前请求的 read-after-write 一致性依赖于异步 watch 自愈
...
如果您希望快速理解“一个请求进入后，这个插件的主要处理流程是什么”，可先阅读本节：
...
- 若整表 refresh 期间已经收到更晚的 `watch` 事件，当前 refresh 结果必须丢弃并再刷一次；旧 list 结果不能把内存快照回滚成更早状态
...
- 一旦规则 `watch` 意外断开，这份“引用关系快照”就不再是 authority；删除协调器必须重新等待重连后的新快照落地，不能沿用断线前那份历史 ready 状态
...
- 优先承认平台边界，不构造“近似事务 / 近似索引”的业务层补偿
- 保持唯一数据来源，不为了绕过限制又把同一份数据拆成两边分别维护
- 如果 Halo CMS 后续补齐原生能力，再顺势切换到平台能力，而不是在插件里长期堆过渡方案

下面这些问题在当前阶段统一按已知问题处理：要么属于 Halo CMS 平台限制，要么继续实现的成本明显高于收益。

### 匹配规则树节点暂不支持键盘重排

这条限制是当前阶段的明确取舍，不属于遗漏实现。

平面资源列表的键盘重排语义相对直接：焦点留在被移动项的拖动句柄上，继续按上下方向键即可微调顺序。  
但匹配规则树不是简单线性列表，它还同时带着：

- 兄弟节点前后重排
- 拖入条件组内部
- 根节点与子节点不同层级
- 焦点在“当前节点本身 / 拖动句柄 / 目标落点”之间如何表达

如果现在补上一套仅覆盖部分路径的键盘重排，容易出现以下结果：

- 方向键语义和视觉插入线对不上
- 读屏提示说不清当前会移动到同级、跨层级还是进入某个组
- 焦点落点不稳定，连续操作体验反而更差
...
- 平面资源列表继续提供完整键盘重排
- 匹配规则树节点仅提供鼠标拖拽重排
- 在补齐键盘重排前，树节点拖动句柄不应暴露成可聚焦按钮；否则键盘用户会遇到“可聚焦但不可操作”的伪能力
- 文档明确承认这是当前成本边界，不用半套键盘语义冒充“已支持”

如果后续要补这块，应该把它当成一项单独设计来做，至少先明确：
...
这里需要先明确：当前删除代码片段并不是“一次请求里把所有相关资源同时改完”，而是分步完成。
...
这并不是插件刻意留下的空档，而是 Halo 当前并未直接提供以下能力：

- 跨多个扩展资源（extension）的单事务原子提交
- 字段级 `partial update / merge patch`（部分更新 / 合并补丁）

因此这里的目标不是构造“近似事务”的表象，而是在 Halo 现有能力范围内把这条链路做稳：
...
这项限制本质上不来自插件额外约束，而是取决于运行时能否拿到模板信息。
...
- 把这条限制显式写进 `README.md`，并用用户可理解的方式说明
...
- `.github/workflows/ci.yaml`
  ...
  - 负责 PR 的主检查，包括插件构建、测试，以及发版版本号约束
  - 普通 PR 可以照常改代码，但不能修改 `gradle.properties` 里的 `version`；如果修改了，`ci.yaml` 会直接失败
  - 只有带 `release` label 的 PR 才允许修改 `gradle.properties` 里的 `version`，且版本号必须是递增的语义化版本；不满足时同样由 `ci.yaml` 拦截
- `.github/workflows/release-stable-plugin.yml`
  - 负责带 `release` label 的 PR 合并后的正式发版流程
  - 会校验合并后的 `main` 是否仍然对应这次 release PR、提升 `CHANGELOG.md` 的 `Unreleased`、提交 changelog commit、创建 GitHub Release

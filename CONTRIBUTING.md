...
- 控制台若复用 watch-driven 快照作为读模型，写口成功后也必须同步把已保存资源回灌进这份可见快照；不要把“当前请求自己的 read-after-write”也赌给异步 watch 自愈
...
- 若整表 refresh 期间已经收到更晚的 `watch` 事件，当前 refresh 结果必须丢弃并再刷一次；不能让旧 list 结果把内存快照回滚成更早状态
...
下面这些问题在当前阶段统一按已知问题处理：要么属于 Halo CMS 平台限制，要么继续硬做的成本明显高于收益。
...
- 平面资源列表继续提供完整键盘重排
- 匹配规则树节点仅提供鼠标拖拽重排
- 在补齐键盘重排前，树节点拖动句柄不应再暴露成可聚焦按钮；否则键盘用户会遇到“能聚焦却不能操作”的假能力

...
前端这块最容易写着写着就乱。  
一开始图快，把“列表数据、编辑草稿、提交参数”揉在一起，短期看很方便，后面基本都会出问题。  
所以这里先把边界画清楚，再谈交互细节。
...
- 新建弹窗同样要复用共享草稿控制器（draft controller），统一“初始值（`baseline`）/ 是否已修改（`dirty`）/ 提交时使用的快照（`submit snapshot`）”这几个语义；不要把这些逻辑重新塞回组件本身
...
下面这些交互细节不是“刚好现在这么写”，而是已经收敛下来的设计约束。  
如果后面要改，请把整条语义一起考虑，不要只改其中一个按钮或一个局部状态：
...
- 凡是异步收敛、失败可重试、事件驱动刷新的后台流程，优先使用控制器 / 协调器 / 监听（`controller / reconciler / watch`）这套平台原语
...
- 一旦资源进入“删除中”状态，控制台读接口、单项写接口、排序读模型和运行时装载都不应再把它当成“可见资源”；否则 `finalizer`（终结器）的最终一致删除会在前端界面（UI）上表现成滞后一拍
- 运行时执行链路优先消费独立的运行时模型，而不是把扩展资源（extension）存储实体继续直接传给过滤器 / 处理器（`filter / processor`）
...
下面这些问题不是“没注意到”，而是当前确实更接近 Halo 平台边界，或者继续往下做的成本明显高于收益。  
所以先明确记在这里，避免以后重复踩坑。
...
这一条本质上不是插件自己的规则，而是运行时上下文能不能拿到的问题。

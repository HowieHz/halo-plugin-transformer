...
- 现在关系只认 `TransformationRule.snippetIds` 这一处，不再在别的地方重复维护
- Halo 当前没有直接提供“按集合成员反查”的原生字段查询能力
...
- `annotations / labels` 承载结构化关系数据

### 删除代码片段是分步完成的，不是跨资源原子事务

现在的删除流程是分步完成的，不是“一次请求里把所有相关资源同时原子改完”。
...
这条链路是**分步完成、最后才会一致**的，不是跨多个扩展资源（extension）的一笔原子事务。  
原因不是插件这里偷懒没收口，而是 Halo 当前并没有直接提供：

- 跨多个扩展资源（extension）的单事务原子提交
- 字段级 `partial update / merge patch`（部分更新 / 合并补丁）

所以这里的设计目标不是伪造事务语义，而是在 Halo 现有能力内做到“现在最好”：
...
这一条本质上不是插件自己定的规则，而是运行时上下文能不能拿到的问题。

...
Halo 自带的全局页面注入，更适合“整站统一加一段内容”这类场景。  
...
这个插件后续在规则能力、控制台体验和运行时处理上都继续往前做了一大步。

## 核心理念

这个插件想解决的，不只是“能改页面”，还包括下面这些事：

- 规则表达尽量灵活，别一遇到稍复杂的场景就写不出来
- 配置过程尽量直接，能在保存前发现的问题就别拖到保存后
...
这一条需要提前说清楚，免得你配了半天，以为是插件没生效：

- `TEMPLATE_ID` 匹配依赖页面正确暴露 `_templateId`
- 对 Halo 自带页面，或者正确接入这套上下文的插件页面，这条链路通常都能稳定工作
- 如果第三方页面没有暴露 `_templateId`，插件就拿不到可靠的模板身份
- 遇到这种情况时，建议优先改用路径这类更稳定的条件

## 常见模板 ID

如果你要配置“模板 ID 匹配”，下面这些值可以直接拿来用。

...
- 支持两种规则编辑方式：
    - 简单模式
    - 高级模式
- 支持单项导出 / 导入，以及代码块、注入规则各自独立的批量导出 / 导入
- 导入 / 导出面向“可移植的可编辑内容”，不会携带 `id`、排序、系统元数据，也不会携带规则与代码块的关联关系
...
- 右侧编辑面板支持进入“批量操作”模式；进入后会清空当前单项选中，只保留当前标签页
...
- 导出的包裹层使用 `version`、`resourceType`、`data`
    - 单项代码块：`resourceType = "snippet"`
    - 单项规则：`resourceType = "rule"`
    - 批量代码块：`resourceType = "snippet-batch"`，`data.items` 为代码块数组
...
> - 注入规则也不会导出 / 导入 `snippetIds`；规则导入后如果需要关联代码块，请在控制台里重新关联

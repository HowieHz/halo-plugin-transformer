...
它的重点不只是“能转换”，还包括尽量把运行时成本、规则可维护性和后台编辑体验一起收紧：

- 运行时规则快照由 `watch` 驱动，并带自动重连与退避重试，减少请求路径上的临时回源
- 正则表达式会复用编译结果，避免高频请求下重复 `Pattern.compile`
- 匹配规则会先做布尔最小化，再参与路径预筛与实际匹配，尽量减少无效判断
- 删除代码片段走 `finalizer + reconciler` 收敛链路，避免把跨资源清理堆进同步接口
- 前后端双重校验会尽量把问题拦在写入前，而不是等到运行时才“悄悄不生效”
- UI/UX 细节极致优化，无障碍支持优化。
- 简单模式能立即定位多个错误，高级模式尽最大可能定位高亮行错误，能从空对象不看文档构建出完整规则树。前后端聚能校验正则错误。直接用 API 能从空对象，不看文档不看已有实现，通过错误信息完整构造。

特别感谢 [HowieHz](https://github.com/HowieHz) 能将这个插件的雏形开源出来，提供了一个非常好的起点。本插件在原有基础上推到了一个新的深度，相信您在使用过程中也能体会到这些改进带来的价值。
...
- 支持三种转换模式：
    ...
    - `CSS 选择器`
- 支持可组合的匹配规则：
    - 页面路径匹配（以斜杠 `/` 开头的页面路径，如 `/archives/page/2`）
    - 模板 ID 匹配（附录：[常见模板 ID](#附录常见模板-id)）
    - `AND` / `OR`
    - 不满足本组/项（`NOT`）
    - 条件组嵌套
- 支持两种规则编辑方式：
    - 简单模式
    - 高级模式
- 支持单项导出 / 导入，以及代码片段、转换规则各自独立的批量导出 / 导入
- 导入 / 导出面向“可移植的可编辑内容”，不会携带 `id`、排序、系统元数据，也不会携带规则与代码片段的关联关系
- 导出的 JSON 会自动附带 `$schema`，方便在支持 JSON Schema 的编辑器中获得提示与校验
- 支持按规则决定是否输出 `PluginTransformer start/end` 注释标记
...
| `selector` | `TransformerWebFilter`       | 通过 CSS 选择器匹配并处理所有命中的元素         |
...
> 因此 `remove` 模式下无需关联代码片段；保存时会自动清空“关联代码片段”这一项，
> 后端也会拒绝这类错误数据。
> 在管理后台中，选择 `remove` 后也不会再显示“关联代码片段”选择区。
...
- 每条规则都可单独配置“注释标记”
- 开启“输出注释标记”后，会输出 `<!-- PluginTransformer start -->` 与 `<!-- PluginTransformer end -->`
...
每条转换规则都会使用一套“匹配规则”来描述“在什么情况下生效”。
...
- 导出内容只包含当前项自身内容，不包含 `id`、关联代码片段、关联规则、排序等系统字段
- 导出的包裹层使用 `version`、`resourceType`、`data`
    - 单项代码片段：`resourceType = "snippet"`
    - 单项规则：`resourceType = "rule"`
    - 批量代码片段：`resourceType = "snippet-batch"`，`data.items` 为代码片段数组
    - 批量规则：`resourceType = "rule-batch"`，`data.items` 为规则数组
...
- 代码片段的“代码内容”编辑框也会显示行号，便于和高级模式一样按行检查内容
...
> 说明：
>
> - 导入 / 导出协议只描述可移植的编辑内容，不是完整资源快照
> - 因此不会携带 `id`、排序、`metadata` 系统态
> - 转换规则也不会导出 / 导入 `snippetIds`；规则导入后如果需要关联代码片段，请在控制台里重新关联

- 代码片段
    - 批量导入时要求 `resourceType = "snippet-batch"`，并且 `data.items` 至少包含 1 项
...
- 转换规则
    - 批量导入时要求 `resourceType = "rule-batch"`，并且 `data.items` 至少包含 1 项
    - 顶层可额外带一个可选的 `$schema`，仅供编辑器提示使用；导入时会忽略它的业务含义
    ...
    - 批量导入时若某一项失败，错误信息会带上“第 N 项”，方便直接定位到坏数据
...
- 匹配规则在运行时会先做一轮布尔最小化，再参与路径预筛、表达式分析与实际匹配
    ...
    - 例如会消去恒等项、折叠常量、去掉双重否定、合并可吸收分支，并在合适场景下应用德摩根变换与因式分解
    - 这样可以让路径预筛更早收窄范围，也减少真正进入 HTML 改写前的无效判断
    - 这轮最小化只服务于运行期与分析期；不会偷偷改写控制台里的编辑草稿，也不会改写持久化存储的原始规则树

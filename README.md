...
- 导出 JSON 的包裹层使用 `version`、`resourceType`、`data`
...
- 当前排序写入语义是 **last-write-wins**：如果有多个管理员同时修改排序，后一次保存会覆盖前一次保存
- 因此当前版本更适合单管理员或低并发后台使用；若需要多人同时安全调整排序，后续应升级为显式 revision / 冲突提示的乐观并发控制

...
- 导出的 JSON 会自动附带 `$schema`，方便在支持 JSON Schema 的编辑器中获得提示与校验
...
> `selector` 模式需要在服务端读取并处理完整 HTML，因此会额外引入响应改写成本。
...
> 但 `CSS 选择器` 模式会先处理所有页面，再继续判断其它条件，配置页也会显示性能警告。

## 注入位置选项

在 `selector` 模式下，可以选择以下位置策略：
...
- 默认开启，便于排查页面中的注入来源
- 每条规则也可单独配置“运行顺序”（`runtimeOrder`）：
    - 只影响**同一执行阶段**内的规则先后，例如 `HEAD` 规则只和 `HEAD` 规则比较
    - 数值越小越先执行
    - 同值时先按规则名称字母序稳定执行；若名称相同或为空，再按规则 `id` 字母序兜底
    - 控制台提供六档快捷值：`0`、`429496729`、`858993458`、`1288490187`、`1717986916`、`2147483645`
    - 也可切换到手动输入，自定义 `0 ~ 2147483647` 任意整数
    - 新建规则默认使用 `2147483645`（最低档），避免刚创建就意外抢到旧规则前面
...
- 左侧“代码块”和“注入规则”列表支持按住拖动手柄调整顺序；拖拽、新建、删除后都会立即尝试保存顺序，保存失败时会提示
...
- 左侧 `rule-order` 仍只控制**控制台列表顺序**；真正运行时顺序由每条规则自己的 `runtimeOrder` 决定
...
- 删除代码块走 Halo `finalizer` 生命周期：先进入 deleting 状态，再由后端协调器摘掉规则引用，最后完成真正删除
- 进入 deleting 的代码块会被视为“当前无法关联”，运行时也不会再继续加载它
- 右侧编辑面板支持把当前代码块 / 注入规则导出为 JSON，按钮位于“启用 / 禁用”左侧
- 新建代码块 / 注入规则时，可先点“导入”把已有模板带入表单
- 新建代码块 / 注入规则时，也可在导入按钮左侧先决定创建后默认是启用还是禁用
- 导出内容只包含当前项自身内容，不包含 `id`、关联代码块、关联规则、排序等系统字段
- 导出的包裹层使用 `version`、`resourceType`、`data`
- 点“导出”后，会先弹出导出窗口；你可以在窗口里选择“复制全部”或“另存为”
- 导出的 JSON 会自动带上 `$schema`；在支持 JSON Schema 的编辑器里，可直接获得字段提示、枚举提示和结构校验
- 规则若当前停留在高级模式：
    - 可成功解析的 JSON 草稿会在导出时自动规范化为 `RULE_TREE`
    - 仍然无效的 JSON 草稿才会继续按 `JSON_DRAFT` 导出，避免把错误悄悄改写成另一棵规则树
- 当简单模式或 JSON 模式当前内容仍有错误时，切换模式会先弹出确认
- 其中 JSON 草稿有误且切回简单模式时，会额外提示这份未保存 JSON 将被覆盖
- 刷新页面时，会回到当前已保存的规则内容；不会继续恢复未保存的编辑模式或 JSON 草稿
- 代码块的“代码内容”编辑框也会显示行号，便于和高级模式一样按行检查内容
- 代码块“代码内容”、匹配规则高级模式、关联代码块/关联规则列表都默认固定高度；内容过多时内部滚动，也可拖动右下角调高
- 带行号的编辑器会关闭自动换行，改为横向滚动；这样行号始终和真实内容行一一对应
- 已修改但尚未保存的字段，会在字段标题旁显示“撤销修改”
- 鼠标悬停在右侧编辑器工具栏上时，会显示撤销 / 重做快捷键提示
- 右侧编辑器支持快捷键：
    - `Ctrl+Z` / `Cmd+Z`：撤销最近一次字段操作
    - `Ctrl+Shift+Z` / `Cmd+Shift+Z`：重做最近一次刚撤销的字段操作
    - `Ctrl+Y`：在 Windows / Linux 下也可重做最近一次刚撤销的字段操作
    - 触发后会弹出一条提示，明确这次撤销 / 重做影响的是哪个字段
- 下拉框需要先点中，再滚动滚轮，才会切换选项；鼠标悬停时不会误切换
- 撤回按钮：
    - 单击，或按住 0.3 秒内松开：回到上一步
    - 按住超过 0.3 秒但不足 2 秒：按钮会开始出现从左到右的提示动画，此时松开仍然只是回到上一步
    - 继续按到 2 秒：自动回到未修改状态
- 在高级模式下：
...
- 匹配规则结构
    ...
    - 其中 `negate` 也要求显式写出 `true` / `false`，不会再按“省略就默认 false”处理
- 字段取值
    - 布尔字段会分别校验是否缺失、以及是否为 `true` / `false`；例如 `negate`
    - 匹配方式必须和规则类型对应
    - 匹配内容不能为空
    - 正则表达式必须合法
...
- 代码块
    ...
    - 布尔字段会校验是否为 `true` / `false`；例如 `enabled`
...
- 注入规则
    ...
    - 顶层布尔字段也会校验是否为 `true` / `false`；例如 `enabled`、`wrapMarker`
    - `matchRule`（匹配规则）会先校验是否还能稳定进入编辑器
    - `matchRule` 里的布尔字段也会校验是否为 `true` / `false`；例如 `negate`
    - `matchRule` 里的枚举字段也会分别校验是否缺失、是否为字符串、以及是否落在允许值里；例如 `type` / `operator` / `matcher`
    - 只要 `matchRule` 根节点仍然是对象，哪怕内部字段缺失、字段类型不对、`children` 不是数组、子节点不是对象等，也会自动切到高级模式，并保留这份原始 `matchRule` JSON 继续修改
    - 只有 `matchRule` 根节点本身不是对象时，才会直接拒绝导入
    - 如果只是字段名写错，导入时会直接忽略这些错误字段
    - 如果只是缺少可补默认值的字段，导入时会自动补上默认配置；例如缺少 `negate` 时会补成 `false`
    - 其中 `GROUP` 缺少 `children` 时会补成空数组 `[]`，并在编辑器里继续提示“不能有空组”，不会偷偷塞一个默认子规则
    - 如果 `type` 合法、只是 `matcher` 不适用于当前类型，导入时会自动回退到该类型的默认匹配方式
    - 空组、空值、非法 `REGEX` 这类仍可在界面里继续修改的问题，允许先导入；导入后编辑器会继续提示，修正后才能保存

#### 保存时校验

创建 / 更新代码块、注入规则时，后端还会再次校验：
...
- 排序映射独立存放在 `snippet-order` / `rule-order` 资源里，不再把顺序写回每条代码块或注入规则本体
- `rule-order` / `snippet-order` 当前都只用于**控制台左侧列表展示顺序**，不参与运行时注入执行优先级
- 运行时规则执行顺序仍由后端当前激活规则快照决定；调整左侧拖拽顺序不会改变注入生效先后

...
一个用于**根据规则修改指定页面内容**的 Halo CMS 插件。

Halo 自带的全局页面注入更偏向站点级场景；这个插件更适合：

- 仅在特定页面路径下转换
- 仅在特定模板 ID 下生效
- 仅对 CSS 选择器命中的元素做插入、替换、移除

特别感谢 [Erzbir](https://github.com/Erzbir) 开源这个插件的雏形，提供了一个很好的起点。本插件在原有基础上推到了一个新的深度，相信您在使用过程中也能体会到这些改进带来的价值。

## 核心理念

这个插件的设计重点不只是“能转换”，还包括：

- 规则表达足够灵活，尽量覆盖各种场景
- 配置体验足够直接，尽量把问题拦在保存之前
- 性能优化足够彻底，尽量把每一笔开销都花在刀刃上
- 界面设计足够细致，尽量减少使用过程中的迷茫与挫败感
...
> `CSS 选择器` 模式需要在服务端读取并处理完整 HTML，因此会额外引入响应改写成本。非必要时，优先使用 `head` / `footer`。

如果你在“全部满足（`AND`）”里加入“页面路径匹配”，插件就能先根据访问路径缩小范围，只处理少量需要改写 HTML 的页面。  
如果规则还不能先按页面路径缩小范围，例如完全没有页面路径条件，或写成“页面路径 OR 模板 ID”这类组合，插件仍然会工作；但 `CSS 选择器` 模式会先处理所有页面，再继续判断其他条件，配置页也会显示性能警告。
...
> 因此 `remove` 模式下无需关联代码片段；保存时会自动清空“关联代码片段”，后端也会拒绝这类错误数据。管理后台中选择 `remove` 后，也不会再显示“关联代码片段”选择区。
...
每条转换规则都会使用一套“匹配规则”来描述“在什么情况下生效”。如果你在接口文档或报错信息里看到 `matchRule`，它对应的就是这里的“匹配规则”。
...
插件支持单项导出 / 导入，以及代码片段、转换规则各自独立的批量导出 / 导入。
...
- 不携带 `id`、排序、系统元数据
- 规则导出 / 导入不携带关联关系
- 导出的 JSON 自动附带 `$schema`，方便在支持 JSON Schema 的编辑器中获得提示与校验
...
如果你需要实现外部工具链、自动生成导入文件，完整的协议细节、字段约束和错误恢复语义见 [CONTRIBUTING.md](./CONTRIBUTING.md)。

## 配置页体验

控制台围绕“少走弯路、少丢上下文”做了不少细节收口：

- 简单模式能即时定位多个错误；高级模式尽可能定位到具体 JSON 行
- 正则表达式会在前后端都做校验，不必等到运行时才发现“悄悄不生效”
...
如果只是缺少可补默认值的字段，会自动补上默认配置；如果仍可继续在界面内修正，则允许先导入再修改。
...
- 删除代码片段时会先进入后台清理流程；控制台列表会立即隐藏这类 deleting 资源，且它们也不会再出现在可编辑 / 可关联集合里，避免出现“明明已经删除却还在列表里”的滞后感
- 若 `CSS 选择器` 规则还不能先按页面路径缩小范围，插件仍会工作，但会带来额外处理开销

更完整的校验清单、规则优化细节、运行时收敛链路和并发写入语义见 [CONTRIBUTING.md](./CONTRIBUTING.md)。

## 已知限制

- `TEMPLATE_ID` 匹配依赖页面正确暴露 `_templateId`
- 对 Halo 自带页面或正确接入该上下文的插件页面，这条链路可以稳定工作
- 如果第三方页面没有暴露 `_templateId`，插件就拿不到可靠的模板身份
- 遇到这种情况时，建议优先改用路径等其他可稳定识别的条件

...
/** Why: 排序映射只保存“显式排过序”的项；未出现的资源默认按 0 处理并排到前面， 同值时再按名称字符序、最后按 id 稳定排序，确保前端即时排序与后端回存后的顺序契约一致。 */
...
/** Why: 一旦用户手动拖拽过顺序，就把当前整组列表固化成显式的 1..n， 这样后续新增项仍会以默认 0 顶到前面，而当前这批已排序项能稳定保持拖拽结果。 */
...
/**
 * Why: 批量导入后的新增资源需要稳定落在当前列表末尾，且保持“用户导入时的成功创建顺序”； 这样排序持久化只关心一个 authoritative
 * list，不必在规则与代码片段两侧各自再维护一套拼接语义。
 */

...
/** Why: 规则读模型进入编辑器前，需要显式补齐规则树与来源状态； 否则简单模式、JSON 模式、撤销与导入导出会围绕不同状态源漂移。 */
...
/** Why: 规则写模型必须只包含后端真正接受的持久化字段； 这里统一把编辑态收敛成 `TransformationRuleWritePayload`，避免 UI 草稿细节泄漏到 API。 */

...
/** Why: 匹配规则来源状态既会参与撤销/重置，也会参与导入导出； 这里统一深拷贝，避免字符串草稿与规则树在多个编辑副本之间相互串改。 */
...
/** Why: 模式切换的核心语义应当只由“目标模式”决定： 切到简单模式一定收敛成规则树，切到高级模式一定生成新的 JSON 草稿， 避免再用额外布尔值把“切换模式和是否覆盖草稿”两层语义搅在一起。 */
